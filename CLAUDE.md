...
npm run start-devnet       # copies config.devnet.ts → config/index.ts + starts dev server → https://localhost:3002
...
npm run cy:run             # or: node scripts/cypress.ts
# Tests hit the remote integration env — no local backend needed.
```

`src/config/index.ts` **must exist** before starting. The `start-*` scripts create it automatically via the `copy-*-config` step. Running `npm run start` directly requires creating it manually.

HTTPS is enabled by default (self-signed cert via `@vitejs/plugin-basic-ssl`). Set `VITE_APP_USE_HTTPS=false` to disable.

Env vars are read from `.env` / `.env.development`. Keys prefixed `VITE_APP_*` are exposed to the client (`VITE_APP_USE_HTTPS`, `VITE_APP_CACHE_BUST`, `VITE_APP_SHARE_PREFIX`, `VITE_APP_GA_ID`).
