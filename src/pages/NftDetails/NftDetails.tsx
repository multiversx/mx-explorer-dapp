...
export const NftDetails = () => {
  ...
  const [nftDetails, setNftDetails] = useState<NftType>();
  const [nftAccounts, setNftAccounts] = useState<NftAccountType[]>([]);
  const [nftAccountsCount, setNftAccountsCount] = useState<number | '...'>(
    '...'
  );
  ...
  const fetchNftDetails = () => {
    getNft(identifier).then((nftData) => {
      if (ref.current !== null) {
        if (nftData.success) {
  ...
  const fetchNftAccounts = () => {
    ...
    Promise.all([
      ...
      if (ref.current !== null) {
        ...
        setDataReady(nftAccounts.success && nftAccountsCount.success);
      }
    });
  };

  useEffect(fetchNftDetails, [identifier]);

  useEffect(fetchNftAccounts, [identifier, searchParams]);
